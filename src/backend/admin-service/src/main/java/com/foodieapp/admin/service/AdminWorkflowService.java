...
public class AdminWorkflowService {
    ...
    @Transactional
    public AdminWorkflow create(AdminWorkflow w) {
        return repo.save(w);
    }

    public AdminWorkflow getById(Long id) {
        return repo.findById(id).orElseThrow(() -> new RuntimeException("Workflow not found: " + id));
    }

    public List<AdminWorkflow> getAll() {
        return repo.findAll();
    }

    public List<AdminWorkflow> getPending() {
        return repo.findByStatus("PENDING");
    }

    @Transactional
    public AdminWorkflow approve(Long id) {
        AdminWorkflow w = getById(id);
